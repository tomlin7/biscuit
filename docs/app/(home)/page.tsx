...
function ThemeToggle() {
  ...
  if (!mounted) {
    return (
      <div className="w-9 h-9 border border-zinc-200 dark:border-zinc-800 rounded bg-zinc-50 dark:bg-zinc-900/50" />
    );
  }
  ...
  return (
    <div className="flex items-center justify-between font-mono text-sm bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded px-4 py-3 w-full max-w-md my-4 text-zinc-800 dark:text-zinc-200">
      <div className="flex items-center gap-2">
        <span className="text-amber-600 dark:text-amber-500 select-none font-bold">
          &gt;
        </span>
        <span>{command}</span>
      </div>
...
export default function HomePage() {
  ...
  const activeImage =
    screenshots.find((s) => s.id === activeScreenshot) || screenshots[0];
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 flex flex-col font-sans transition-colors duration-200">
      ...
      <nav className="sticky top-0 z-50 bg-white/90 dark:bg-zinc-950/90 backdrop-blur-md border-b border-zinc-200 dark:border-zinc-900 px-6 py-4 transition-colors">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <Link
            href="/"
            className="flex items-center gap-2.5 font-mono font-bold text-lg select-none"
          >
          ...
          <div className="hidden md:flex items-center gap-6 font-mono text-sm">
            <Link
              href="/docs"
              className="text-zinc-600 dark:text-zinc-400 hover:text-amber-600 dark:hover:text-amber-500 transition-colors"
            >
              documentation
            </Link>
            <a
              href="https://deepwiki.com/tomlin7/biscuit"
              target="_blank"
            ...
            >
              deepwiki <ExternalLink className="h-3 w-3" />
            </a>
            <a
      ...
      {/* Hero Grid Section */}
      <header className="max-w-7xl mx-auto w-full px-6 py-12 md:py-20 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div className="lg:col-span-4 flex flex-col items-start text-left">
          ...
          <p className="text-md text-zinc-600 dark:text-zinc-400 font-mono leading-relaxed mb-6">
            a fast, extensible, native code editor with agents. lightweight
            &lt;20 mb in size. install and start using in seconds.
          </p>
        ...
        <div className="lg:col-span-8 w-full flex flex-col">
          {/* Tab switches */}
          <div className="flex flex-wrap gap-1.5 mb-4 font-mono text-[10px] uppercase tracking-wider">
            {screenshots.map((s) => (
              <button
                key={s.id}
                onClick={() => {
                ...
                className={`px-3 py-1.5 rounded transition-all cursor-pointer font-bold ${
                  activeScreenshot === s.id
                    ? "bg-zinc-100 dark:bg-zinc-900 text-amber-600 dark:text-amber-500"
          ...
          {/* Screenshot viewport (No border, smooth cross-fade transition, larger layout) */}
          <div
      ...
      <hr className="border-zinc-200 dark:border-zinc-900 max-w-7xl mx-auto w-full px-6" />

      {/* Editor Mockup Feature Board Section */}
      <section className="max-w-7xl mx-auto w-full py-16 px-6">
        <div className="mb-8">
          ...
          <p className="text-zinc-600 dark:text-zinc-400 text-sm font-mono mt-1">
            inspect the implemented progress code files in a simulated biscuit
            editor interface.
          </p>
        </div>
        ...
        <div className="border border-zinc-200 dark:border-zinc-800 rounded-lg overflow-hidden bg-white dark:bg-zinc-950 flex flex-col md:flex-row min-h-[500px] shadow-sm">
          ...
          <div className="flex-1 flex flex-col bg-white dark:bg-zinc-950 font-mono text-sm overflow-hidden">
            ...
            <div className="flex items-center border-b border-zinc-200 dark:border-zinc-900 bg-zinc-50 dark:bg-zinc-900/20 text-xs text-zinc-500">
              <div className="flex items-center gap-2 px-4 py-3 bg-white dark:bg-zinc-950 border-r border-zinc-200 dark:border-zinc-900 font-bold text-amber-600 dark:text-amber-500 ">
            ...
            <div className="p-4 md:p-6 overflow-y-auto max-h-[450px] flex-1 flex select-text">
              ...
              <div className="text-zinc-300 dark:text-zinc-800 pr-4 text-right select-none border-r border-zinc-100 dark:border-zinc-900 w-10">
                {activeFile.lines.map((_, idx) => (
                  <div key={idx} className="leading-6 text-[12px]">
                    {idx + 1}
                  </div>
                ))}
              </div>
              ...
              <div className="pl-4 flex-1 space-y-0 leading-6 text-[13px] overflow-x-auto text-zinc-800 dark:text-zinc-300">
                {activeFile.lines.map((line, idx) => {
                  ...
                  if (isHeader)
                    textColorClass =
                      "text-amber-600 dark:text-amber-500 font-bold text-base border-b border-zinc-100 dark:border-zinc-900 pb-1 mb-2 block";
                  else if (isChecked)
                    textColorClass = "text-zinc-800 dark:text-zinc-200";
                  else if (isPlanned)
                    textColorClass = "text-zinc-400 dark:text-zinc-600";

                  return (
                    <div
                      key={idx}
                      className={`${textColorClass} whitespace-pre`}
                    >
                      {isChecked && (
                        <span className="text-emerald-500 dark:text-emerald-500 font-bold mr-1.5">
                          ✔
                        </span>
                      )}
                      {isPlanned && (
                        <span className="text-zinc-400 dark:text-zinc-600 font-bold mr-1.5">
                          ☐
                        </span>
                      )}
      ...
      <section className="max-w-7xl mx-auto w-full py-16 px-6 grid grid-cols-1 md:grid-cols-2 gap-12 font-mono">
        {/* Getting started */}
        <div>
          ...
          <div className="space-y-6 text-sm text-zinc-600 dark:text-zinc-400">
            <div>
              <p className="text-zinc-800 dark:text-zinc-200 mb-2 font-bold">
                1. installation
              </p>
              <p className="mb-2">
            ...
            <div>
              ...
              <p className="mb-2">
                launch the editor and open a target workspace directory
                instantly:
              </p>
              <pre className="bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded p-3 text-zinc-800 dark:text-zinc-200 text-xs">
                biscuit path/to/src
            ...
            <p className="text-xs">
              see other installation formats (including compiled executable
              releases) in the{" "}
              <a
        ...
        <div>
          ...
          <div className="space-y-4 text-sm text-zinc-600 dark:text-zinc-400">
            <p>
              biscuit is fully open source. contributions of any scope are
              highly appreciated!
            </p>
            ...
            <p className="text-xs pt-2">
              documentation is a work-in-progress. feel free to inspect our code
              palette or ask on our{" "}
              <a
                href="https://deepwiki.com/tomlin7/biscuit"
                target="_blank"
                rel="noopener noreferrer"
      ...
      <footer className="mt-auto border-t border-zinc-200 dark:border-zinc-900 bg-zinc-100/50 dark:bg-zinc-950 py-8 px-6 font-mono text-xs text-zinc-500 dark:text-zinc-600">
        <div className="max-w-7xl mx-auto w-full flex flex-col md:flex-row items-center justify-between gap-4">
          <div>&copy; {new Date().getFullYear()} Tomlin7. MIT License.</div>
